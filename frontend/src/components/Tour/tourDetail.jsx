...
function TourDetail() {
  ...
  const getRecommend = async (tourName) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/recommend?tourName=${encodeURIComponent(
          tourName
        )}`
      );
  ...
  return (
    <div className="flex flex-col gap-10">
      <div className="flex flex-row justify-between pt-10">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-row gap-1 items-center">
            <p className="rounded-md bg-[#01B7F2] p-1 text-white font-semibold">
              {dataTour.discount}
            </p>
        ...
        <div className="flex flex-col gap-5 items-end justify-between">
          ...
          <div className="flex flex-row gap-3">
            ...
            <Button isIconOnly variant="bordered" className="border-[#01B7F2]">
              <ShareIcon />
            </Button>
            <Link to={`/checkout/${dataTour.tourId}`}>
              <Button
                disabled={dataTour.status === false ? true : false}
                className= { `${dataTour.status === true ? "bg-[#01B7F2] text-white" : "text-[#4682B4]"}   font-semibold text-[1.2em]`}
              >
                {dataTour.status === true ? "Đặt ngay" : "Không hoạt động"}
              </Button>
            </Link>
          </div>
        </div>
      ...
      <div>
        <div className="text-xl font-semibold pb-5">Vị trí / Map</div>
                        ...
                        <div className="flip-card-front">
                          <CardBody className="p-0">
                            <div className="relative group">
                              <Link
                                to={`/tourDetail/${recommendTour.tourId}`}
                                onClick={() =>
                                  handleSelectTour(recommendTour.tourId)
                                }
                              >
                                <Image
                                  isZoomed
                                ...
                                <div className="relative flex gap-1">
                                  <CardFooter className="justify-center">
                                    <div className="flex flex-col font-semibold text-lg">
                                      {recommendTour.tourName.length > 10
                                        ? recommendTour.tourName.substring(
                                            0,
                                            20
                                          ) + "..."
                                        : recommendTour.tourName}
                                    </div>
                                  </CardFooter>
                                </div>
                              </div>
